    ...
    with pipeline_launcher(cfg) as pipeline_modules:
        ...
        if cfg.get("test"):
            ...
            if checkpoint_callback:
                ...
                if ckpt_path == "":
                    ckpt_path = None
            # Fall back to the user-supplied `ckpt_path` (e.g. when running test-only
            # without a preceding fit, `best_model_path` is empty and we'd otherwise
            # silently test randomly-initialized weights).
            if not ckpt_path:
                ckpt_path = cfg.get("ckpt_path") or None
                if ckpt_path:
                    command_line_logger.info(
                        f"Using ckpt_path from config for testing: {ckpt_path}"
                    )
            if not ckpt_path:
