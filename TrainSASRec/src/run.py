...
from custom_ESASRec_modified import ESASRecV2
...
def apply_esasrec_stage_requires_grad(model, stage):
    """Stage 0: train backbone (item path); freeze adapter and content embeddings.
    Stage 1: train adapter only.
    Stage 2: train all except item_content_emb.
    """
    ...
    if stage == 0:
        ...
        for p in model.item_content_emb.parameters():
            p.requires_grad = False
        ...
        raise ValueError(f'Unknown ESASRec stage: {stage}')
...
def create_dataloaders(train, validation, config):
    ...
    if config.model in ['SASRec', 'GPT4Rec', 'RNN', 'ESASRecV2']:
...
def create_model(config, item_count):
    ...
    if config.model == 'SASRec':
        ...
        model = SASRec(add_head=add_head, **config.model_params)
    if config.model == 'BERT4Rec':
    ...
    elif config.model == 'ESASRecV2':
        model = ESASRecV2(item_num=item_count, add_head=add_head, **config.model_params)
        if config.ESASRecStage.start_ckpt_path is not None:
            checkpoint_state = torch.load(config.ESASRecStage.start_ckpt_path)['state_dict']
            # Stage0 checkpoint is saved from Lightning module and prefixes weights with `model.`.
            if any(k.startswith('model.') for k in checkpoint_state.keys()):
                checkpoint_state = {
                    k[len('model.'):]: v if k.startswith('model.') else v
                    for k, v in checkpoint_state.items()
                }
            model.load_state_dict(checkpoint_state)
            pce = getattr(config.model_params, "path_to_cont_embeddings", None)
            model.load_embeddings(pce)
        model.stage = config.ESASRecStage.stage
        apply_esasrec_stage_requires_grad(model, model.stage)
...
def training(model, train_loader, eval_loader, config):
    ...
    trainer = pl.Trainer(callbacks=callbacks, accelerator="gpu", devices=1, enable_checkpointing=True,
